...
function state(){
 ...
 $.ajax({
      type:"POST",
	  url:"../include/location/pump_dropdown.php",
	  ...
	  $('#statedd1').append('<option value="'+ data[i].pname +'">'+ data[i].pname+' ('+ data[i].p_product+') </option>');
	   });
	  },
	  complete:function(){}
    });
...
$('#districtdd1').append("<option>Loading.......</option>");
   $.ajax({
   type:"POST",
	  url:"../include/location/price_dropdown.php?sid="+sid,
	  contentType:"application/json; charset=utf-8",
      dataType:"json",  
      success: function(data){
      $('#districtdd1').empty();
   //$('#districtdd1').append("<option value='0'>----Select Branch-----</option>");	  
	...
	// $('#districtdd1').append('<option value="'+ data[i].p_price +'selected">'+ data[i].p_price+'</option>');
	   $('#districtdd1').val(data[i].p_price);
	   $('#districtdd2').val(data[i].p_product);
	 $('#dd2').val(data[i].tmw_open_m);
