...
if (mysql_num_rows($query_run) >=1)  
{
  $mngs='Product name '.$txt1.' already exist';
	   ...
	   header("location:tan3a.php?tp=".$roxi."&msg2=".$mngs);
				 exit();
...
if(empty($txt1) || empty($txt2)|| empty($txt3) || empty($txt6)  ){
	...
	$mngs='Please fill the form completely';
	   ...
	   header("location:tan3a.php?tp=".$roxi."&msg2=".$mngs);
	      ...
	      mysql_query("Update e_tank SET tmw_open_stock ='$n_stock' , new_stock='$txt5' ,date_add_stock='$dat' where tank_code='$roxi' ");
	
	$query = mysql_query(" INSERT INTO hist_tank( `tank_name`, `tank_prod`, `tank_liters`,old_stock,new_stock,current_stock,tank_code,date_stock) VALUES  ('{$txt1}','{$txt2}','{$txt3}','{$txt4}','{$txt5}','{$n_stock}','{$roxi}','$dat')") or die(mysql_error()); 
	 
  	
	$mng='Successfully submitted';
	
	   header("location:tan3a.php?tp=".$roxi."&msg=".$mng);
				 exit();
...
<p><span>Current Stock</span><input name="txt4" class="form-control" value="0" placeholder="0"   type="text" style="width:200px;"  readonly >
</p>
	
			 <?php }else{
				 
	?>		
<p><span>Current Stock</span><input name="txt4" class="form-control"  data-validation="required"  type="text" style="width:200px;" value="<?php echo $trx['tmw_open_stock']?>" readonly required>
</p>
...
<p><span>New Stock</span><input  name="txt5" class="form-control"  data-validation="required"  type="text" style="width:200px;" required>
</p>
...
<p><span>Date</span><input name="txt6" class="form-control" data-validation="required" id="datepicker" autocomplete="off"  data-validation-error-msg="Input Date" value="" type="text" style="width:200px;" required/>
		...
		<br><br>
		<?php /*   $dt=date('Y-m-d');    if($trx['tmw_open_stock'] === $tr['tank_liters'] ){  ?>
		
			  <div class="alert alert-success" >
 <strong><i class="fa fa-success-circle fa-2x"></i></strong>Tank <?php echo $tr['tank_name'] ?> is Full.
</div>
		
		
		<?php }else{ ?>
 <input   type="submit"  class="btn btn-primary" name="submit" value="Add New Stock"/>
...
$h = mysql_query("Select * FROM hist_tank where DATE(cur_date) = '$dat' ") or die(mysql_error()); 
        ...
        <tr class="record">
		    ...
		    <td><br><?php echo $tr['old_stock']; ?></td>
			<td><br><?php echo $tr['new_stock']; ?></td>
			<td><br><?php echo $tr['current_stock']; ?></td>
			<td><br><?php echo $tr['date_stock']; ?></td>
			 <td>&nbsp;&nbsp;&nbsp;&nbsp; <a href="#" id="<?php echo $tr['tank_code']; ?>" class="delbutton" title="Click To Delete"><span class="fa fa-trash"></a>
