...
$pin = 'SAL/'.''.generatePIN();
  ...
  $query = "SELECT `p_name` FROM `e_pump` WHERE `prod_name`='{$txt1}'";
$query_run = mysql_query($query);


if (mysql_num_rows($query_run) >=1)  
{
  $mngs='Pump name '.$txt1.' already exist';
	
	   header("location:add_pmp.php?msg2=".$mngs);
				 exit();
...
if(empty($txt1) || empty($txt2)|| empty($txt3) ){
	...
	$query = mysql_query(" INSERT INTO e_pump ( `p_name`, `p_product`, `p_price`) VALUES  ('{$txt1}','{$txt2}','{$txt3}')") or die(mysql_error());
	 
  	
	$mng='You Have Successfully added '.$txt1.'';
	
	   header("location:add_pmp.php?msg=".$mng);
				 exit();
...
<p><span>Pump Name</span><input name="txt1" class="form-control"  data-validation="required"  type="text" style="width:200px;" required>
</p>
...
$h = mysql_query("Select * FROM e_pump ") or die(mysql_error()); 
        ...
        <tr class="record">
            ...
            <td><br><?php echo $tr['p_name']; ?></td>
		    <td><br><?php echo $tr['p_product']; ?></td>
			<td><br><?php echo $tr['p_price']; ?></td>
			<!-- <td>&nbsp;&nbsp;&nbsp;&nbsp; <a  rel="facebox"  href="pmp_edit.php?erp_id=<?php //echo $tr['p_id']; ?>" class=" btn-sm">
          <i class="fa fa-pencil"></i> 
        </a> 
	
		</td> -->
		<td>&nbsp;&nbsp;&nbsp;&nbsp; <a href="#" id="<?php echo $tr['p_id']; ?>" class="delbutton" title="Click To Delete"><span class="fa fa-trash"></a>
