...
<h3 class="box-title"><center><?php echo $tr['cb_name'] ;?> &nbsp;&nbsp;&nbsp;&nbsp; <?php $kk=$_GET['pes'];  echo $kk; $sheet=$tr['cb_name']; ?></center></h3>
...
$dat1 = date('d/m/Y', strtotime($d1)); echo $dat1;  ?>
			...
			<td><br><?php echo $tr['ban_stat']; ?></td>
            <td><br><?php echo $tr['ban_deb']; ?></td>
			<td><br><?php echo $tr['ban_cred']; ?></td>
...
$p2=$_GET['pes'];
	$k2 = mysql_query("SELECT sum(ban_cred),sum(ban_deb)  FROM ban_transac WHERE cb_name='$sheet' AND DATE_FORMAT(ban_dat,'%M %Y')='$p2'  ") or die(mysql_error()); 
			 
			 $trk = mysql_fetch_array($k2);
			    ?>
  <th> <?php  echo $trk['sum(ban_deb)'];  ?></th>
  <th> <?php  echo $trk['sum(ban_cred)'];  ?></th>
   </tr>
