<?php  include '../include/header.php';?>
    ...
    <ul class="sidebar-menu">
     <li class="header">MAIN NAVIGATION</li>
        ...
        <li class="active"><a href="stoa.php"><i class="fa fa-circle-o"></i>Add Stock</a></li>
        <li><a href="stoc.php"><i class="fa fa-circle-o"></i>Stock Sales</a></li>
         </ul>
            </li>
 ...
 */
   ...
   $qy2 = mysql_query("SELECT stock_name ,stock_qty, stock_code FROM e_stock WHERE stock_name='{$txt1}'");
   
   
   $cx = mysql_fetch_array($qy2);
   $up_stock = $txt2 + $cx['stock_qty'];
   $st_name= $cx['stock_name'];
   $st_code= $cx['stock_code'];
...
if(empty($txt1) || empty($txt2)|| empty($txt3)  ){
	...
	$mngs='Please fill the form completely';
	   ...
	   header("location:stoa.php?msg2=".$mngs);
	      ...
	      mysql_query("Update e_stock SET stock_qty ='$up_stock' , last_qty_upd='$dat' where stock_name='$txt1' ");
	
	$query = mysql_query(" INSERT INTO stock_hist ( stock_name,stock_code,stock_qty,stock_date) VALUES  ('{$txt1}','{$st_code}','{$txt2}','{$dat}')") or die(mysql_error());
...
<p><span>Stock Name</span>

<select name="txt1" class="form-control"  data-validation="required" style="width:200px;" required>  
<option  value="" selected>--Select Stock Name---</option>
<?php $hix2 =mysql_query("select stock_name FROM e_stock")  or die(mysql_error()); 
			 
			 while ($tox = mysql_fetch_array($hix2)){ 
		?>

<option  value="<?php echo $tox['stock_name']; ?>"><?php echo $tox['stock_name']; ?></option>
...
<p><span>Stock Quantity (pieces)</span><input name="txt2" class="form-control" style="width:200px;"  data-validation="required" type="text" required>
</p>

<p><span>Date</span><input  name="txt3" class="form-control" data-validation="date required" data-validation-format="mm/dd/yyyy" autocomplete='off' id="datepicker"  data-validation="required"  type="text" style="width:200px;" required>
</p>
 ...
 <input   type="submit"  class="btn btn-primary" name="submit" value="Add Stock"/>
               ...
               <hr>
               <h3>List Of Stock Added</h3>
<table id="example1" class="table table-bordered table-hover">
    	<thead>
    	<tr>
            
        <th width="10%">Stock Name  </th>
        <th width="10%">Stock Quantity (pieces)</th>
...
$h = mysql_query("Select * FROM stock_hist") or die(mysql_error()); 
        ...
        <tr class="record">
			...
			<td><br><?php echo $tr['stock_qty']; ?></td>
			<td><br><?php echo $tr['stock_date']; ?></td>
		<td>&nbsp;&nbsp;&nbsp;&nbsp; <a  rel="facebox" href="stoa2.php?sto_id=<?php echo $tr['hist_id']; ?>" class=" btn-sm">
          <i class="fa fa-pencil"></i> 
        </a> 
	
		</td><td>&nbsp;&nbsp;&nbsp;&nbsp; <a href="#" id="<?php echo $tr['hist_id']; ?>" class="delbutton" title="Click To Delete"><span class="fa fa-trash"></a>
