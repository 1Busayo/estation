...
$kool = mysql_query("Select   acc_tran_name ,  sum(acc_tran_debit)  FROM acc_tran WHERE acc_tran_ledger LIKE '%Ass%' AND acc_tran_date  Between '$da' AND '$da2'  GROUP BY acc_tran_name  ") or die(mysql_error()); 
 $toor = mysql_fetch_array($kool);
$kool2 = mysql_query("Select    acc_tran_name ,  sum(acc_tran_credit) FROM acc_tran WHERE acc_tran_ledger LIKE '%liabi%' AND acc_tran_date  Between '$da' AND '$da2'  GROUP BY acc_tran_name ") or die(mysql_error()); 
			
	$toor2 = mysql_fetch_array($kool2);

	$kool3 = mysql_query("Select own_percent ,own_name , acc_tran_name ,  sum(acc_tran_debit) , sum(acc_tran_credit)  FROM acc_tran , owners_prop WHERE acc_tran.own_code=owners_prop.own_code AND acc_tran_date  Between '$da' AND '$da2' GROUP BY acc_tran_name  ") or die(mysql_error()); 
	
	$toor3 = mysql_fetch_array($kool3);

	       if($toor==NULL || $toor2==NULL || $toor3==NULL){
			   
			   Echo "<p>No Records Found </p><p> Please Ensure You have Prepared Or Created Your Revenue And Expenses Account For The Particular Date,Month ,Year You Are Looking For.<p> ";
...
<tr>
		<td><center><b>Assets</b></center></td>
		<td></td>
		<td></td>
		</tr>
<?php
			 
$h1 = mysql_query("Select   acc_tran_name ,  sum(acc_tran_debit)  FROM acc_tran WHERE acc_tran_ledger LIKE '%Ass%' AND acc_tran_date  Between '$da' AND '$da2'  GROUP BY acc_tran_name  ") or die(mysql_error()); 

	
			 $su=0;
		while($tr = mysql_fetch_array($h1))
		{
		?>
...
<tr>
 <td><?php echo $tr['acc_tran_name']; ?></td>

	
        <td><?php echo $tr['sum(acc_tran_debit)'];   $rev=$su+=$tr['sum(acc_tran_debit)'];$GLOBALS['rev'];    ?> </td>
...
<tr>
 <td></td>
 <td></td>
	  <td> <b><?php  echo $rev; ?> </b></td>
</tr>



<tr>
 <td><center><b>Liabilities</b></center></td>
 <td><p></p></td>
...
$h2 = mysql_query("Select    acc_tran_name ,  sum(acc_tran_credit) FROM acc_tran WHERE acc_tran_ledger LIKE '%liabi%' AND acc_tran_date  Between '$da' AND '$da2'  GROUP BY acc_tran_name ") or die(mysql_error()); 
...
<tr>
 <td><?php echo $tr['acc_tran_name']; ?></td>
		...
		<td><p></p></td>
  <td><p> <?php echo $tr['sum(acc_tran_credit)']; $exp=$sum+=$tr['sum(acc_tran_credit)'];$GLOBALS['exp'];   }  ?> </p></td>

</tr>

<tr>
 <td><center><b>Owners Equity</b></center></td>
 <td></td>
<td></td>
 <tr>
 <?php
		 
$h3 = mysql_query("Select own_percent ,own_name , acc_tran_name ,  sum(acc_tran_debit) , sum(acc_tran_credit)  FROM acc_tran , equ_ity WHERE acc_tran.own_code=equ_ity.own_code AND acc_tran_date  Between '$da' AND '$da2' GROUP BY acc_tran_name  ") or die(mysql_error()); 
	
			 $su=0;
		while($tr = mysql_fetch_array($h3))
		{
		?>
        ...
        <?php
		//owners own equity		  
		$capital= $tr['sum(acc_tran_credit)'];?>

 <?php   $netcalc = $tr['own_percent']/100*$net;?>
	
       

<?php  $with=$tr['sum(acc_tran_debit)']; ?>
<?php  $increase=$netcalc-$with; ?>
	

</tr>

<tr>
 <td><?php echo $tr['own_name']; ?></td>
 <td><p></p></td>
	
        <td><p> <?php echo $eqty=$capital+$increase; $GLOBALS['eqty'];  ?></p></td>
...
<?php
 ...
 <td><p></p></td>
        ...
        <td><p><b><?php  $toteq=array_sum($data);  echo $toteq + $exp; ?></b></p></td>
</tr>
...
function fnExcelReport() {
    ...
    if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
        if (window.navigator.msSaveBlob) {
            ...
            navigator.msSaveBlob(blob, 'Balance sheet.xls');
        }
    } else {
		//window.open( data_type + ',' + encodeURIComponent(tab_text));
        $('#test').attr('href', data_type + ', ' + encodeURIComponent(tab_text));
        $('#test').attr('download', 'Balance sheet.xls');
    }
