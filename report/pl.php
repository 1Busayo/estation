...
echo "<Center><p><b>Income Statement</b></p><b>". $d1 = date('d F Y ', strtotime($a)) ." - ".  $d2 = date('d F Y', strtotime($b))."</b></Center><br>";



$kool = mysql_query("Select   c_prod ,  sum(amount) FROM e_sales WHERE  e_date  Between '$da' AND '$da2'   GROUP BY c_prod  ") or die(mysql_error()); 

    $toor = mysql_fetch_array($kool);
	
	
$kool2 = mysql_query("Select    acc_tran_name ,  sum(acc_tran_debit) FROM acc_tran WHERE acc_tran_ledger LIKE '%EXPENSES%' AND acc_tran_date  Between '$da' AND '$da2'  GROUP BY acc_tran_name ") or die(mysql_error()); 
	 
		$toor2 = mysql_fetch_array($kool2);

	       if($toor==NULL || $toor==NULL){
...
<table class="table table-bordered table-hover" align="center">
	<thead>
		...
		<th> <center><b>REVENUE</b></center> </th>
...
$h = mysql_query(" Select   c_prod ,  sum(amount) FROM e_sales WHERE e_date  Between '$da' AND '$da2'   GROUP BY c_prod ") or die(mysql_error()); 
			 $su=0;
...
<tr>
 <td><?php echo $tr['c_prod']; ?>&nbsp;&nbsp;</td>

	
        <td><?php echo (int) $tr['sum(amount)'];   $rev=$su+=$tr['sum(amount)'];$GLOBALS['rev'];  ?> &nbsp;&nbsp;</td> 
...
<tr>
 <td><center><b>EXPENSES</b></center></td>
 <td><p></p></td>
 <td><p></p></td>
...
$k = mysql_query("Select   cust_name,  deb_amt  FROM exp_pay WHERE pay_dat  Between '$da' AND '$da2'   ") or die(mysql_error()); 
			 
			
		$sum=0;	 
		while($tr = mysql_fetch_array($k))
		{
		?>

<tr>
 <td><?php echo $tr['cust_name']; ?>&nbsp;&nbsp;</td>
  <td><?php echo $tr['deb_amt']; $exp=$sum+=$tr['deb_amt'];$GLOBALS['exp']; ?>&nbsp;&nbsp;</td> 
...
<?php  
      ...
      $rh = mysql_query("SELECT * FROM  cust_pay, customerdetails  WHERE cust_pay.c_id=customerdetails.c_id and date_paid Between '$da' AND '$da2' ") or die(mysql_error()); 
			 $smxx=0;
			 while($rtr = mysql_fetch_array($rh)){

?>

 <td><?php echo $rtr['c_name']; ?> (<?php echo $rtr['date_paid']; ?>) &nbsp;&nbsp;</td>
  <td><?php echo $rtr['total_amount']; 	 $sal=$smxx+=$rtr['total_amount'];$GLOBALS['sal']; ?>&nbsp;&nbsp;</td> 
...
<td><p></p></td>
		...
		<td><b><?php echo $toxz = $exp + $sal; ?></b></td>
            
</tr>
<tr>
        <td><b>Net Income</b></td>
<td><p></p></td>
		<td><b><?php $net= $rev-$toxz;  echo (int) $net; ?></b></td>
...
function fnExcelReport() {
    ...
    if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
        if (window.navigator.msSaveBlob) {
            ...
            navigator.msSaveBlob(blob, 'Income-Statement'+trandate+'.xls');
        }
    } else {
		//window.open( data_type + ',' + encodeURIComponent(tab_text));
        $('#test').attr('href', data_type + ', ' + encodeURIComponent(tab_text));
        $('#test').attr('download', 'Income-Statement'+trandate+'.xls');
    }
