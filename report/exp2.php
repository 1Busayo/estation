...
echo "<Center><p><b>Expense</b></p><b>". $d1 = date('d F Y ', strtotime($a)) ." - ".  $d2 = date('d F Y', strtotime($b))."</b></Center>";


$kool = mysql_query(" select * FROM exp_pay WHERE  pay_dat Between '$da' AND '$da2' ") or die(mysql_error()); 
$toor = mysql_fetch_array($kool);

	       if($toor==NULL){
			   
			   Echo "<p>No Records For  Found </p>";
...
$h = mysql_query(" select * FROM exp_pay WHERE  pay_dat Between '$da' AND '$da2' ") or die(mysql_error()); 
	 ...
	 <p hidden>  	<?php
		...
		$hx = mysql_query(" select sum(deb_amt) as yui FROM exp_pay WHERE  pay_dat Between '$da' AND '$da2' ") or die(mysql_error()); 
			 
		

		$trx = mysql_fetch_array($hx);
...
function fnExcelReport() {
    ...
    if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
        if (window.navigator.msSaveBlob) {
            ...
            navigator.msSaveBlob(blob, 'Expense-Report-'+trandate+'.xls');
        }
    } else {
		//window.open( data_type + ',' + encodeURIComponent(tab_text));
        $('#test').attr('href', data_type + ', ' + encodeURIComponent(tab_text));
        $('#test').attr('download', 'Expense-Report-'+trandate+'.xls');
    }
