...
echo "<Center><p><b>Trial Balance</b></p><b>". $d1 = date('d F Y ', strtotime($a)) ." - ".  $d2 = date('d F Y', strtotime($b))."</b></Center>";


$kool = mysql_query("Select  IFNULL(acc_tran_name,'Total') as acc_tran_name , sum(acc_tran_debit) , sum(acc_tran_credit) FROM acc_tran Where acc_tran_date Between '$da' AND '$da2' GROUP BY acc_tran_name  with ROLLUP") or die(mysql_error()); 
$toor = mysql_fetch_array($kool);

	       if($toor==NULL){
			   
			   Echo "<p>No Records Found </p><p> Please Ensure You have Prepared Or Created  Account For The Particular Date,Month ,Year You Are Looking For.<p> ";
...
<table class="table table-bordered table-hover" align="center">
	<thead>
		...
		<th style="width:290px;"><b>Debit(Dr)</b></th>
		<th style="width:290px;"><b>Credit(Cr)</b></th> 
		</thead>
<tbody>
<?php
$h = mysql_query("Select  IFNULL(acc_tran_name,'Total') as acc_tran_name , sum(acc_tran_debit) , sum(acc_tran_credit) FROM acc_tran Where acc_tran_date Between '$da' AND '$da2' GROUP BY acc_tran_name  with ROLLUP") or die(mysql_error()); 
...
<tr>
 <td align="left"><?php echo $tr['acc_tran_name']; ?>&nbsp;&nbsp;</td>
 
 <td align="left"><?php if($tr['sum(acc_tran_debit)'] === "0" ){  echo ""; } else { echo $tr['sum(acc_tran_debit)']; } ?></td>
 <td align="left"><?php if($tr['sum(acc_tran_credit)'] === "0" ){  echo ""; } else { echo $tr['sum(acc_tran_credit)']; } ?></td>
</tr>
		   <?php }  ?>
  </tbody>
</table>
...
function fnExcelReport() {
    ...
    if (msie > 0 || !!navigator.userAgent.match(/Trident.*rv\:11\./)) {
        if (window.navigator.msSaveBlob) {
            ...
            navigator.msSaveBlob(blob, 'Trial balance.xls');
        }
    } else {
		//window.open( data_type + ',' + encodeURIComponent(tab_text));
        $('#test').attr('href', data_type + ', ' + encodeURIComponent(tab_text));
        $('#test').attr('download', 'Trial balance.xls');
    }
